...
int choose_format(EGLDisplay egl_disp, EGLConfig egl_conf, const char *conf_str)
{
...
int main(int argc, char *argv[])
{
	...
	if(brightness != 0) {
		rc = screen_set_window_property_iv(screen_win, SCREEN_PROPERTY_BRIGHTNESS, &brightness);
		if (rc) {
	...
	if(contrast != 0) {
		rc = screen_set_window_property_iv(screen_win, SCREEN_PROPERTY_CONTRAST, &contrast);
		if (rc) {
	...
	if(saturation != 0) {
		rc = screen_set_window_property_iv(screen_win, SCREEN_PROPERTY_SATURATION, &saturation);
		if (rc) {
...
#else
  EGLint pbuf[] = {EGL_WIDTH, size[0], EGL_HEIGHT, size[1], EGL_NONE};
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/graphics/branches/release-2.1.x/apps/screen/demos/gles2-gears/gles2-gears.c $ $Rev: 836876 $")
#endif
