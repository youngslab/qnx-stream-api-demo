...
/* location of the Alpha uniform used in the fragment shader */
static GLuint alpha_loc;
...
/* Parameter set from the command line */
GLfloat fg_alpha = 1.0;
...
#include "gears.fs"
    ...
    alpha_loc = glGetUniformLocation(program, "fgAlpha");
}
...
void gears_init(int width, int height)
{
...
void gears_draw(void)
{
    ...
    glUniform1fv(alpha_loc, 1, (GLfloat*)&fg_alpha);
    ...
    angle = fmodf(angle + (2.0f * deg2rad), twopi);
    view_rotx = fmodf(view_rotx + (1.0f * deg2rad), twopi);
}

#if defined(__QNXNTO__) && defined(__USESRCVERSION)
#include <sys/srcversion.h>
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/graphics/branches/release-2.1.x/apps/screen/demos/gles2-gears/gears.c $ $Rev: 805537 $")
#endif
